...
const handlePointerLeave = (event: PointerEvent) => {
  ...
  // Use client coordinates to compute offsets, so animations
  // are correct even when the card is scrolled or transformed.
  const rect = card.getBoundingClientRect();
  ...
  createSmoothAnimation(
    ...
    wrap,
    () => {
      // After easing back to the center, fully reset state
...
onMounted(() => {
  ...
  createSmoothAnimation(ANIMATION_CONFIG.INITIAL_DURATION, initialX, initialY, card, wrap, () => {
    // Ensure variables are centered after intro animation
